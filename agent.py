...
class Agent:
    ...
    def use_tools(self) -> None:
        messages = [
            {"role":"user", "content":prompts.PLANNING_AGENT_PROMPT.format(goal=self.memory.goal, previous_action=self._get_previous_action(), tools=self._generate_tool_descriptions())},
        ]
